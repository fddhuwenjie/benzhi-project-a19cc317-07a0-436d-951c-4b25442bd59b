...
func detachedCommandContext(ctx context.Context) context.Context {
...
func (s *Service) CreateTrial(ctx context.Context, c CreateTrialCommand) (CommandResult, error) {
	...
	r, err := s.repo.Create(detachedCommandContext(ctx), t, c.RequestID, createFingerprint, domain.CreationAuditPayload(t), c.ActorID, response)
	if err != nil {
		return CommandResult{}, err
	}
	return result(r), nil
...
func (s *Service) AddObservation(ctx context.Context, trialID string, c ObservationCommand) (CommandResult, error) {
	...
	r, err := s.repo.Transact(detachedCommandContext(ctx), trialID, c.RequestID, fp, c.ExpectedRevision, func(t *domain.ClearanceTrial) (store.Mutation, error) {
